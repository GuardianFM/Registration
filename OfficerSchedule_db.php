...
//        $sql = "SELECT tbl_jobaccept.Status,tbl_jobaccept.shiftid, tbl_dutyschedule.* FROM `tbl_dutyschedule` JOIN tbl_jobaccept ON tbl_jobaccept.shiftid=tbl_dutyschedule.ID WHERE tbl_jobaccept.officerid='$id' AND DATE(tbl_dutyschedule.DutyStart)>'$CurrentDate' AND tbl_jobaccept.Status!='Approve'";
        ...
        if (mysqli_num_rows($result) > 0) {
          ...
          <tr class="table-row" style="text-align:center;">
          <td class="col col-1" data-label="S No.">' . $x . ' </td>
          <td class="col col-1" data-label="Duty Type">' . $row['Event_Type'] . ' </td>
          <td class="col col-1" data-label="Description"><a href="#" id="job_description" data-id="' . $row['OID'] . '" style="color:green;"><i class="fa fa-file-text-o"></i></a> </td>
...
//    $row = mysqli_fetch_array($result);
    ...
    $array = shiftInfo($shiftid);

    $sql = "SELECT * FROM `tbl_dutyschedule` where `DutyFinish` >'" . $array[0] . "' AND `Officer_id`='$officerid'";

    $result = mysqli_query($connect, $sql);

    $row = mysqli_fetch_array($result);
    if (mysqli_num_rows($result) > 0) {
        //If Officer had shift on that day
        echo 'You already have shift to cover at ' . $row['Location'] . ' ' . date("Y-m-d", strtotime($row['DutyStart'])) . ' ' . date("H:i:s", strtotime($row['DutyStart'])) . '-' . date("H:i:s", strtotime($row['DutyFinish']));
    } else {
        //If Officer dont have shift on that day
...
//       echo $sql;
    ...
    $sql = "SELECT * FROM `tbl_requestlogs` WHERE `ID`='$id'";
    $result = mysqli_query($connect, $sql);
...
function shiftInfo($id) {
    ...
    $sql = "SELECT * FROM `tbl_dutyschedule` where `ID`='$id'";

    $result = mysqli_query($connect, $sql);

    $row = mysqli_fetch_array($result);
    if (mysqli_num_rows($result) > 0) {
        $data[0] = $row['DutyStart'];
        $data[1] = $row['DutyFinish'];
        return $data;
    } else {
        echo 'No Data Found';
    }
